...
int main() {
    ...
    std::vector<int> pages = {1, 2, 3, 4, 2, 5, 3, 1, 6, 7, 2, 2, 6};
    numFrames = 3;
    numPages = pages.size();
        ...
        if (!found) {
            ...
            while (true) {
                if (!referenced[hand]) {
                    ...
                    referenced[hand] = true; // Đánh dấu là đã tham chiếu ngay lập tức
            ...
            // Nếu trang đã có trong bộ nhớ, đánh dấu là đã tham chiếu
            referenced[hand] = true; // Đánh dấu là đã tham chiếu ngay lập tức
        }
    }
